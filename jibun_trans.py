...
def getKeywords(conn):
    ...
    cur.execute("SELECT full_address, id FROM store WHERE doro_post_address IS NULL AND emd_name IS NULL")

    return cur.fetchall()

def getTotalLength(conn):
    cur = conn.cursor()
    cur.execute("SELECT count(*) FROM store WHERE doro_post_address IS NULL AND emd_name IS NULL")
    result = cur.fetchone()
...
def doSearch(keywords, conn):
    ...
    for row in keywords:
        count += 1 
        keyword = getKeyword(row['full_address']).replace(",", "")
        id=row['id']
...
def getFullAddressWithJuso (juso):
    ...
    if type(juso) is not list and juso is not None:
        fullAddress = juso['roadFullAddr']
    elif type(juso) is list:
        fullAddress = juso[0]['roadFullAddr']
...
def insertData (id, fullAddress, conn):
    print("excute query... " + "UPDATE store SET doro_post_address = '" + fullAddress + "' where id = " + str(id))
    cur = conn.cursor()
    cur.execute("UPDATE store SET doro_post_address = '" + fullAddress + "' where id = " + str(id))
    conn.commit()
